...
def scan_and_delete_found_folders(base_directory):
    ...
    # Confirmarea finală
    confirmation = input(f"\n🗑️  Dorești să ștergi toate folderele găsite? (scrie 'DA' pentru confirmare): ")

    if confirmation.upper() == 'DA':
        print(f"\n🚀 Încep ștergerea {found_count} foldere...")
        print("="*60)
        ...
        for i, folder in enumerate(folders_to_delete, 1):
            try:
                print(f"[{i:3}/{found_count}] Șterg {folder['name'][:40]:<40}", end=" ")
        ...
        print(f"\n🎉 OPERAȚIUNE COMPLETĂ!")
        print(f"{'='*60}")
        print(f"   ✅ Foldere șterse: {deleted_count}/{found_count}")
...
def main():
    ...
    print("• Muta în backup folderele găsite online")
    print("="*50)
